# DAG which creates DDS (with cleaned data) layer in Vertica database
...
def create_dds_layer():
    query_create_dds_layer = """
        ...
        with conn.cursor() as cur:
            try:
                ...
                if result[0] == 100:
                    ...
                    cur.execute(query_create_dds_layer)
            except:
...
with DAG(
    ...
    with TaskGroup("cread_DDS_tables") as create_DDS:
        t21 = PythonOperator(task_id="create_dds_layer", python_callable=create_dds_layer, dag=dag)
    end = DummyOperator(task_id="end")
    
    start >> create_DDS >>  end
